...
class Solution:
    def b2l(self, board: list) -> list:
        ...
        # 每个元素-1,让value和index匹配:
        for i in range(len(listbd)):
            listbd[i] -= 1
        return listbd

    def snakesAndLadders(self, board: list) -> int:
        listbd = self.b2l(board)
        l = len(listbd)
        ...
        while q:
            ...
            for i in range(idx+1, idx + 7):
                ...
                next = i if listbd[i] == -2 else listbd[i]
                if next == l - 1:
                    return step + 1
            ...
            print(f"index = {idx+1}, step = {step+1}")
            print([_[0]+1 for _ in q])
        return -1
