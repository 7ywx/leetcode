...
class Solution:
    def reconstructQueue(self, people: List[List[int]]) -> List[List[int]]:
        ...
        for i in range(len(people)):
            h, k = people[i]
            if k == 0:
                sorted_indexs.insert(0, i)
                continue

            insert_index = 0
            while k > 0:
                if people[insert_index][0] >= h:
                    k -= 1
                insert_index += 1

            sorted_indexs.insert(insert_index, i)

            # if insert_index == len(sorted_indexs):
            #     sorted_indexs.append(i)
            # else:
            #     sorted_indexs.insert(insert_index, i)

        return [people[sorted_index] for sorted_index in sorted_indexs]
