...
def game_analysis(activity: pd.DataFrame) -> pd.DataFrame:
    ...
    # 返回修改后的活动表
    """
    在这个例子中，函数game_analysis被定义了两次。当调用game_analysis(df)时，它实际上调用了第二个game_analysis函数，而忽略了第一个函数。 这种现象被称为函数重载，但是在这个例子中，两个函数的参数和返回值是相同的，唯一的区别是函数的实现。因此，Python根据函数的参数类型和名称来区分不同的函数重载版本。 在这种情况下，Python根据函数的定义顺序来确定调用哪个函数。第一个game_analysis函数在第二个game_analysis函数的前面定义，所以当使用相同的参数调用时，它将被忽略，Python将自动调用后续定义的函数。 
    """
    print(2)
    return activity
