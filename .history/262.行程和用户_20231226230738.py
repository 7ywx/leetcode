...
def trips_and_users(trips: pd.DataFrame, users: pd.DataFrame) -> pd.DataFrame:
    ...
    for index,row in trips.iterrows():
        print((users[users['users_id'] == row['client_id']]['banned'].value[0]=='No'))
        #print((users[users['users_id'] == row['client_id']]['banned']=='No') & (users[users['users_id'] == row['driver_id']]['banned'] == 'No'))
