...
def top_three_salaries(employee: pd.DataFrame, department: pd.DataFrame) -> pd.DataFrame:
    ...
    # it_salaries = e_d[e_d['Department'] == 'IT']['Salary']
    # sales_salaries = e_d[e_d['Department'] == 'Sales']['Salary']
    it_salaries = e_d[e_d['Department'] == 'IT']['Salary']
    sales_salaries = e_d[e_d['Department'] == 'Sales']['Salary']
    threshold = max(it_salaries.nth(3), sales_salaries.nth(3))
    result = e_d[(e_d['Department'] == 'IT') & (e_d['Salary'] > threshold)] | \
            e_d[(e_d['Department'] == 'Sales') & (e_d['Salary'] > threshold)]
    # e_d = e_d.groupby('Department')['Salary'].apply(lambda x: x[x > e_d['Salary'].nlargest(3).min()])
    print(result)
