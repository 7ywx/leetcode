...
def find_employees(employee: pd.DataFrame) -> pd.DataFrame:
    # 根据经理id和id进行内连接，连接的列名为'id'
    data = pd.merge(employee, employee, on='id', how='inner', suffixes=('_x', '_y'))
    print(data)
    # 选取工资_x大于工资_y的记录
    high_earners = data[data['salary_x'] > data['salary_y']]
    # 选取name_x列并重命名为'Employee'
    employees = high_earners[['name_x']].rename(columns={'name_x': 'Employee'})
    # 返回'Employee'这一列的数据
