...
def order_scores(scores: pd.DataFrame) -> pd.DataFrame:
    '''rank 函数的参数包括：ascending: 排序方式，True为正序，False为降序。
...
def order_scores(scores: pd.DataFrame) -> pd.DataFrame:
    '''对分数进行排序并返回排序后的DataFrame。

    参数:
    ascending: bool, 排序方式，True为升序，False为降序。
    method: str, 排序方法，包括'average', 'min', 'max', 'first', 'dense'。
    na_option: str, 处理NaN的方式，包括'keep', 'top', 'bottom'。
    pct: bool, 是否返回百分比排名，True为返回百分比排名，False为返回绝对排名。
    axis: int, 指定排序的轴，0为按行排序，1为按列排序。

    返回:
    pd.DataFrame: 排序后的分数DataFrame。
    '''
    # 对分数进行排序，使用dense方法从高到低进行排名
    scores['rank'] = scores['score'].rank(method='dense', ascending=False)
...
# 示例使用
# 假设scores是一个包含'score'和'id'列的DataFrame
# order_scores(scores)
