...
class Solution:
    ...
    def maxTaskAssign(self, tasks: List[int], workers: List[int], pills: int, strength: int) -> int:
        ...
        dp = [[0] * (len(workers)) for _ in range(len(tasks))]
        ...
        #         if tasks[i] <= workers[j] + strength and pills > 0:
        #             pills -= 1
        #             dp[i][j] = max(dp[i][j]+1,dp[i][j-1])
        #             self.print_2d_array(dp)
        i,j=0
        while i<len(tasks):
            while j<len(workers):
                if tasks[i] <= workers[j]:
                    dp[i][j] = max(dp[i][j-1],dp[i][j]+1)
                    j += 1
                elif tasks[i] <= workers[j] + strength and pills > 0:
                    pills -= 1
                    dp[i][j] = max(dp[i][j]+1,dp[i][j-1])
                # # 不使用药丸，当前工人无法完成当前任务
                # dp[i][j] = max(dp[i][j-1],dp[i-1][j],dp[i-1][len(workers)])#max(dp[i][j-1],dp[i-1][j],dp[i-1][len(workers)])
                # print(str(i)+' '+str(j))
                # self.print_2d_array(dp)
                ...
                #     print(str(i)+' '+str(j))
                #     self.print_2d_array(dp)
                # elif tasks[i-1] <= workers[j - 1] + strength and pills > 0:
                #     pills -= 1
                #     dp[i][j] = max(dp[i][j]+1,dp[i][j-1])
                #     dp[i][j] += 1
                #     workers[j-1] = -1
        ...
        # 返回最大完成任务数
        print(min(len(workers),len(tasks),max(dp[-1])))
        return min(len(workers),len(tasks),max(dp[-1]))
        # tasks.sort()
        # workers.sort()
